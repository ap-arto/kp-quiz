...
function App() {
  ...
  function persist(next: State, nextLanguage: Language) {
    try {
      localStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({ language: nextLanguage, stats: next.stats }),
      )
  ...
  useEffect(() => {
    if (selected && screen === 'quiz') nextButton.current?.focus()
  }, [selected, screen])
  ...
  const mistakes =
    session?.questions.filter((q) => session.answers[q.id] !== q.correctAnswerId) ?? []
    ...
    <>
      ...
      <header className="site-header">
        ...
        <div className="language-switch" role="group" aria-label={t('language')}>
          <button
            type="button"
            lang="uk"
            aria-label={t('languages.uk.name')}
            aria-pressed={language === 'uk'}
            onClick={() => changeLanguage('uk')}
          >
            {t('languages.uk.short')}
          </button>
          <button
            type="button"
            lang="pl"
            aria-label={t('languages.pl.name')}
            aria-pressed={language === 'pl'}
            onClick={() => changeLanguage('pl')}
          >
            {t('languages.pl.short')}
          </button>
          <button
            type="button"
            lang="en"
            aria-label={t('languages.en.name')}
            aria-pressed={language === 'en'}
            onClick={() => changeLanguage('en')}
          >
            {t('languages.en.short')}
          </button>
        </div>
      </header>
      <main id="main" className={screen === 'home' ? 'home' : 'session'}>
              ...
              {data.categories.map((c, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 id={`category-${c.id}`}>{t(`categoryNames.${c.id}`)}</h3>
                    ...
                    <button
                      className="primary"
                      onClick={() => start(c.id, 'quick')}
                      aria-label={t('quickLabel', { category: t(`categoryNames.${c.id}`) })}
                    >
                      <span>
                        {t('quick')}
                    ...
                    <button
                      ...
                      onClick={() => start(c.id, 'full')}
                      aria-label={t('fullLabel', { category: t(`categoryNames.${c.id}`) })}
                    >
        ...
        {screen === 'quiz' && session && current && category ? (
          <>
            <button className="back-button" onClick={() => dispatch({ type: 'home' })}>
            ...
            <progress
              ...
              value={session.index + Number(Boolean(selected))}
            ...
            <section className="question-card" aria-labelledby="question-title">
              ...
              <h1 ref={heading} tabIndex={-1} id="question-title" lang={data.language}>
                {current.question}
              </h1>
              <div className="answers" role="group" aria-labelledby="question-title">
                {current.answers.map((answer, index) => {
                  ...
                  const revealedClass =
                    selected && isCorrect ? ' correct' : isSelected ? ' incorrect' : ''
                  return (
                    <button
                      key={answer.id}
                      ...
                      onClick={() =>
                        dispatch({ type: 'answer', questionId: current.id, answerId: answer.id })
                      }
                    >
                      ...
                      <span className="answer-copy">
                        <span lang={data.language}>{answer.text}</span>
              ...
              <div aria-live="polite" aria-atomic="true">
                {selected ? (
                  <aside
                    className={`feedback ${selected === current.correctAnswerId ? 'feedback-correct' : 'feedback-incorrect'}`}
                  >
                    <strong>
                      {selected === current.correctAnswerId
                        ? `✓ ${t('correct')}`
                        : `✕ ${t('incorrect')}`}
                    </strong>
                    <h2>{t('fact')}</h2>
                    <p lang={data.language}>{current.fact}</p>
                  </aside>
                ) : null}
              </div>
              {selected ? (
        ...
        {screen === 'result' && session ? (
          <>
            <section className="result-summary" aria-labelledby="result-title">
              ...
              <h1 id="result-title" ref={heading} tabIndex={-1}>
                {t('result')}
              </h1>
              <p>{t('resultCopy')}</p>
              <div className="score">
                ...
                <p>
                  {t('score')}:{' '}
                  <b>{t('scoreCount', { correct, total: session.questions.length })}</b>
                </p>
              </div>
              <button className="primary" onClick={() => dispatch({ type: 'home' })}>
                {t('back')}
                <span aria-hidden="true">→</span>
              </button>
            </section>
            {mistakes.length ? (
              <section className="mistakes" aria-labelledby="mistakes-title">
                ...
                {mistakes.map((q) => (
                  <article key={q.id} className="mistake-card">
                    <h3 lang={data.language}>{q.question}</h3>
                    <p className="missed-answer">
                      ✕ {t('yourAnswer')}:{' '}
                      <span lang={data.language}>
                        {q.answers.find((a) => a.id === session.answers[q.id])?.text}
                      </span>
                    </p>
                    <p className="right-answer">
                      ✓ {t('correctAnswer')}:{' '}
                      <strong lang={data.language}>
                        {q.answers.find((a) => a.id === q.correctAnswerId)?.text}
                      </strong>
                    </p>
                    <p className="mistake-fact" lang={data.language}>
                      {q.fact}
                    </p>
                  </article>
                ))}
